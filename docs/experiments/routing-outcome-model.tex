...
The correctness constraint remains unresolved at $\tau=0$. Historical correctness is 0.3867 on
378,069 labelled rows. For the ridge top-three rule, DM gives 0.3616, below the floor, while the
augmented score gives 0.4135, above it. Boosting gives the same qualitative disagreement,
0.3680 versus 0.4862. Reporting only the augmented value would choose the estimator that gives
the convenient answer.

The corrected total-rule sweep preserves all 450,567 rows and keeps its two estimators separate.
The fitted direct duration and correctness curves are nondecreasing, as
Proposition~\ref{prop:mono} predicts. Their AIPW corrections are not: augmented correctness is
0.4135 at $\tau=0$, falls to 0.3085 at $\tau=0.2$, and returns to 0.3869 at $\tau=0.3$.
Consequently the smallest feasible point is $\tau=0.3$ by DM but $\tau=0$ by AIPW. At the DM
candidate, 247,363 rows use the highest-correctness fallback; duration is 85.72 days by DM and
78.64 by AIPW, while correctness is 0.3995 and 0.3869. There is no resolved $\tau^\star$.
...
$\tau$ calibration (Cor.~\ref{cor:feasible}) & Built with a total highest-correctness fallback, finite-prediction checks, and a common support-restricted population. \emph{Does not resolve}: the direct and augmented estimators disagree about whether the constraint binds (\S\ref{sec:ev-provisional}).\\
\midrule
